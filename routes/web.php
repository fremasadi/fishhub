...
Route::middleware(['role:peternak'])->group(function () {
    ...
    Route::get('peternak/pengambilan/{pengambilan}', [PengambilanController::class, 'show'])->name('peternak.pengambilan.show');

    Route::post('peternak/pengambilan/{pengambilan}/ready', [PengambilanController::class, 'markReady'])->name('peternak.pengambilan.ready');

    Route::post('peternak/pengambilan/{pengambilan}/confirm', [PengambilanController::class, 'confirm'])->name('peternak.pengambilan.confirm');
...
Route::middleware('auth')->group(function () {
    ...
    // Checkout from cart
    Route::post('/payment/checkout', [PaymentController::class, 'processCheckout'])->name('payment.checkout');

    // Show payment page
    Route::get('/payment/{pesanan}', [PaymentController::class, 'show'])->name('payment.show');

    // Check payment status (AJAX)
    Route::get('/payment/{pesanan}/check-status', [PaymentController::class, 'checkStatus'])->name('payment.check-status');
